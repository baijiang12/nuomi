...
class Base extends Controller
{
    ...
    public function _initialize() {
        ...
        //首页大图
        $cats1 = $this->getRecommendCats1();
        //print_r($cats1);
        //print_r($citys[name]);
        $this->assign('citys', $citys);
        $this->assign('city', $this->city);
        $this->assign('cats', $cats);
        $this->assign('cats1', $cats1);
    ...
    //获取首页推荐当中的首页大图的数据
    public function getRecommendCats1() {
        $parentIds = $sedcatArr = $recomCats = [];
        $cats = model('Deal')->getNormalRecommendCategory1ByParentId(2);

        foreach ($cats as $sedcat){
            $parentIds[]=$sedcat->image;
        }
        return $parentIds;
